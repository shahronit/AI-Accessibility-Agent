...
4. Under **Environment Variables**, add the same keys you use in `.env.local` (at minimum one LLM key such as `GEMINI_API_KEY`). Copy names and optional values from `.env.example`.
   ...
   - Add the `NEXT_PUBLIC_FIREBASE_*` + `FIREBASE_SERVICE_ACCOUNT_JSON` vars only if you want sign-in + persistent per-user history. Without them, the app runs in guest mode for everyone.
...
3. After the first deploy, fill in the `sync: false` secrets in **Service → Environment**:
   ...
   | --- | --- | --- |
   | `NEXT_PUBLIC_FIREBASE_API_KEY`, `NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN`, `NEXT_PUBLIC_FIREBASE_PROJECT_ID`, `NEXT_PUBLIC_FIREBASE_APP_ID` | Optional | Firebase web config. Inlined into the client bundle at build time so users can sign in with Email/Password, Google, or GitHub. Without these, /signin still renders but provider buttons error — guest mode is unaffected. |
   | `FIREBASE_SERVICE_ACCOUNT_JSON` | Optional | Server-side Admin SDK credential. Without it, the app keeps running but **all** users are bucketed under the shared `guest` Firestore document — no per-user history persists server-side. |
   | `ANTHROPIC_API_KEY` and/or `GEMINI_API_KEY` and/or `ASSEMBLYAI_API_KEY` | **At least one** | Powers `/api/ai-*` and `/api/chat`. |
   | `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN` | Optional but recommended | Enables per-IP rate limiting, 10-min scan cache, and the `/history` + `/report/[scanId]` URL-keyed scan store. When unset, all three degrade gracefully. |
   | `IBM_CHECKER_ENABLED` | Optional (default `true`) | Set to `"false"` to skip the IBM Equal Access pass and shave ~3–5s per scan. |

4. Open the **`https://<service-name>.onrender.com`** URL Render assigns. Guest mode works immediately; sign-in works once the Firebase env vars above are set.
...
3. Set secrets — at minimum one AI key (sign-in is opt-in):

   ```bash
   fly secrets set \
     ANTHROPIC_API_KEY=...                  # or GEMINI_API_KEY

   # Optional — enable Firebase sign-in + per-user history:
   fly secrets set \
     NEXT_PUBLIC_FIREBASE_API_KEY=... \
     NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN=... \
...
docker run --rm -p 3000:3000 \
  ...
  -e FIREBASE_SERVICE_ACCOUNT_JSON='{...inline JSON...}' \
  a11yagent
```
...
The app is **guest-default**: every route works without signing in. Sign-in is **opt-in** and unlocks per-user scan history that survives across browsers/devices.

- **Authentication:** [Firebase Authentication](https://firebase.google.com/docs/auth) with three providers — **Email/Password**, **Google**, and **GitHub**. Sign-in flows mint a long-lived `__session` HttpOnly cookie via [Firebase session cookies](https://firebase.google.com/docs/auth/admin/manage-cookies).
- **Database:** [Cloud Firestore](https://firebase.google.com/docs/firestore) (Native mode). Layout: `users/{uid}`, `scans/{scanId}`, and `scans/{scanId}/pages/{pageId}` (subcollection — no foreign-key dance).

### Firebase Console setup (one-time)

1. Create a project at [console.firebase.google.com](https://console.firebase.google.com).
2. **Authentication → Sign-in method**: enable **Email/Password**, **Google**, and **GitHub**.
   - For GitHub, register a new OAuth app at [https://github.com/settings/developers](https://github.com/settings/developers) with the callback URL `https://<your-firebase-project>.firebaseapp.com/__/auth/handler`. Paste the GitHub Client ID + secret into the Firebase Console.
3. **Firestore Database → Create database → Native mode** (region: `us-central1` is fine for most setups). The composite indexes the app needs are checked in at [`firestore.indexes.json`](./firestore.indexes.json) — deploy them with `firebase deploy --only firestore:indexes` if you use the Firebase CLI.
4. **Project settings → Service accounts → Generate new private key**. Paste the entire JSON into `FIREBASE_SERVICE_ACCOUNT_JSON` in `.env.local` (single line is fine — `\n` inside `private_key` is auto-normalized).
5. **Project settings → General → Your apps → Web app** → copy the four config values into the matching `NEXT_PUBLIC_FIREBASE_*` vars in `.env.local`.

### Migrate from the old SQLite store

If you previously ran the app with the SQLite database (`./data/a11yagent.db`), copy it into Firestore once with the included script:

```bash
# Set the Firebase Admin credentials (one of):
export FIREBASE_SERVICE_ACCOUNT_JSON='{...}'
# or
export GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json

# Dry run to see counts:
MIGRATION_DRY_RUN=1 npm run migrate:firestore

# For real (idempotent — safe to re-run):
npm run migrate:firestore
```

The script preserves every primary key, so existing scan ids stay valid in any bookmarks or report URLs.
...
A11yAgent is **guest-default** in CI — `/api/scan` is open and needs no
auth bypass token. Add these only if you want to enable specific
features:
...
| `FIREBASE_SERVICE_ACCOUNT_JSON` | Optional | Only needed if you want CI scans to persist as an authenticated Firestore user. |

The workflow does **not** require Upstash, Firebase, or IBM Equal Access
credentials — rate limiting, persistence, and IBM checking degrade
